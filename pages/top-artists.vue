  ...
  <div>
    ...
    <div class="mt-12">
      <div class="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4">
        <NuxtLink :to="`/artist/${artist.id}`"  class="group flex flex-wrap md:px-4 py-3 rounded hover:bg-slate-800 cursor-pointer w-full" v-for="(artist, key) in top_artists.items" :key="key">
          <div class="md:w-4/12 w-5/12">
            <SpotifyImage :to="`/artist/${artist.id}`" :image-src="artist.images[1].url" />
          </div>          
          <div class="md:w-8/12 md:px-5 pl-4 w-7/12">
